...
function useTransportValue(value) {
  ...
  const whichResult = React.useSyncExternalStore(
    () => () => {
    },
    () => 0 /* client */,
    () => valueRef.current === CLEAN ? 0 /* client */ : equality.equal(value, valueRef.current) ? 0 /* client */ : 1 /* server */
  );
...
function useWrapTransportedQueryRef(queryRef) {
  ...
  const unwrapped = internal.unwrapQueryRef(queryRef);
  React.useEffect(() => {
    if (!isTransported)
      return;
    if (cacheKey) {
      if (unwrapped.disposed) {
        internal.getSuspenseCache(client).add(cacheKey, unwrapped);
      }
    }
  });
  React.useEffect(() => {
...
function wrap(useFn, transportKeys) {
  return (...args) => {
    ...
    const forTransport = React.useMemo(() => {
      const transport = {};
...
function WrapApolloProvider(TransportProvider) {
  const WrappedApolloProvider3 = ({
    ...
    const clientRef = React.useRef(undefined);
    ...
    return /* @__PURE__ */ React__default.default.createElement(react.ApolloProvider, { client: clientRef.current }, /* @__PURE__ */ React__default.default.createElement(
      TransportProvider,
      {
...
exports.skipDataTransport = skipDataTransport;
exports.teeToReadableStream = teeToReadableStream;
exports.useWrapTransportedQueryRef = useWrapTransportedQueryRef;
